(function(root,factory){
  ...
  const SPOKEN_ONES={...ONES,won:1,to:2,too:2,for:4,ate:8};
  function normalize(v){return String(v||"").toLowerCase().replace(/[–—-]/g," ").replace(/[^a-z0-9#' ]+/g," ").replace(/\s+/g," ").trim()}
  function spokenNumber(v,allowHomophones=false){const s=normalize(v);if(/^\d+$/.test(s))return Number(s);const ones=allowHomophones?SPOKEN_ONES:ONES;let total=0,found=false;for(const w of s.split(" ")){if(w in ones){total+=ones[w];found=true}else if(w in TENS){total+=TENS[w];found=true}}return found?total:null}
  ...
  function playerMentions(text,roster){
    ...
    for(const p of roster||[]){const name=normalize(p.name),jersey=String(p.jersey??"").trim();let at=-1;
      ...
      if(jersey){const numberPhrase="(?:\\d+|(?:zero|oh|o|one|won|two|to|too|three|four|for|five|six|seven|eight|ate|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)(?:\\s+(?:one|won|two|to|too|three|four|for|five|six|seven|eight|ate|nine))?)";const re=new RegExp(`(?:#\\s*|(?:number|jersey)\\s+)(${numberPhrase})(?=\\s|$)`,"g");let m;while((m=re.exec(clean))){if(spokenNumber(m[1],true)===Number(jersey)&&(at<0||m.index<at))at=m.index}}
  ...
  return {normalize,spokenNumber,applyCorrections,distance,playerMentions,parsePositions,extractYards,interpretVoiceCommand};
});
