...
const PRODUCT_CODES = { statkeeper: "statkeeper_season", team_pro: "team_pro_season", team_pro_upgrade: "team_pro_season" } as const;
...
async function analytics(admin:any,event_name:string,user_id:string,team_id:string,properties:any={}){const {error}=await admin.from("analytics_events").insert({event_name,user_id,team_id,properties});if(error)console.warn("analytics event skipped",event_name,error.message)}
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const input=await req.json().catch(()=>({})),teamId=String(input?.teamId||""),requestedPlan=String(input?.plan||"");
  if(!/^[0-9a-f-]{36}$/i.test(teamId)||!["statkeeper","team_pro"].includes(requestedPlan))return json(req,{error:"Choose a valid team plan"},400);
  const [{data:team},{data:member},{data:entitlement}]=await Promise.all([admin.from("teams").select("id,owner_user_id,name").eq("id",teamId).maybeSingle(),admin.from("team_members").select("is_admin,is_statkeeper,status").eq("team_id",teamId).eq("user_id",user.id).maybeSingle(),admin.from("team_entitlements").select("tier,paid_access_starts_at,paid_access_ends_at,access_source,complimentary").eq("team_id",teamId).maybeSingle()]);
  const authorized=team?.owner_user_id===user.id||(member?.status==="active"&&(member.is_admin||member.is_statkeeper));if(!team||!authorized)return json(req,{error:"Only this team's statkeeper can purchase a plan"},403);
  if(entitlement?.complimentary||["founder_comp","internal_test"].includes(entitlement?.access_source))return json(req,{error:"This team already has complimentary Team Pro access"},409);
  const now=Date.now(),paidEnd=entitlement?.paid_access_ends_at?Date.parse(entitlement.paid_access_ends_at):0,currentStatkeeper=entitlement?.tier==="statkeeper"&&paidEnd>now,purchaseKind=requestedPlan==="team_pro"&&currentStatkeeper?"team_pro_upgrade":requestedPlan,priceId=PRICE_IDS[purchaseKind as keyof typeof PRICE_IDS],productCode=PRODUCT_CODES[purchaseKind as keyof typeof PRODUCT_CODES];
  await analytics(admin,"checkout_started",user.id,teamId,{requested_plan:requestedPlan,purchase_kind:purchaseKind,current_tier:entitlement?.tier||null});
  const {data:product,error:productError}=await admin.from("billing_products").select("id").eq("code",productCode).eq("active",true).single();if(productError||!product)throw new Error("The selected product is not available");
  const {data:subscription,error:subscriptionError}=await admin.from("team_subscriptions").insert({team_id:teamId,purchaser_user_id:user.id,product_id:product.id,provider:"stripe",provider_price_id:priceId,status:"pending"}).select("id").single();if(subscriptionError||!subscription)throw subscriptionError||new Error("Could not start checkout");
  const appUrl="https://hootson.github.io/sideline-stats/",params=new URLSearchParams();params.set("mode","payment");params.set("line_items[0][price]",priceId);params.set("line_items[0][quantity]","1");params.set("success_url",`${appUrl}?checkout=success&session_id={CHECKOUT_SESSION_ID}`);params.set("cancel_url",`${appUrl}?checkout=cancelled`);params.set("client_reference_id",teamId);if(user.email)params.set("customer_email",user.email);params.set("metadata[team_id]",teamId);params.set("metadata[purchaser_user_id]",user.id);params.set("metadata[subscription_id]",subscription.id);params.set("metadata[purchase_kind]",purchaseKind);params.set("payment_intent_data[metadata][team_id]",teamId);params.set("payment_intent_data[metadata][subscription_id]",subscription.id);params.set("payment_intent_data[metadata][purchase_kind]",purchaseKind);
