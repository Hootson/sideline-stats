...
function openAuth(){
  ...
  $("#signupPlanChooser")?.classList.toggle("hidden",!!pendingTeamInviteToken());
...
async function openOwnerDashboard(){
  ...
  closeAuth();$("#ownerDashboardModal")?.classList.remove("hidden");const target=$("#ownerMetrics");target.innerHTML='<div class="inline-note">Loading traction data…</div>';
  try{
...
function openPlans(trialWelcome=false){
  ...
  $$('.plan-checkout').forEach(btn=>{const plan=btn.dataset.plan;btn.disabled=access.active&&access.status==="active"&&access.tier===plan;btn.textContent=btn.disabled?"Current Plan":access.tier==="statkeeper"&&plan==="team_pro"?"Upgrade to Team Pro — $25":plan==="statkeeper"?"Buy Statkeeper — $14.99":"Buy Team Pro — $39.99"});
...
async function beginPlanCheckout(plan,button){
  ...
  renderPlanSelection(plan);const message=$("#plansMessage");if(message){message.classList.remove("error");message.textContent="Connecting securely to Stripe…"}
...
function renderGameArea(){
  ...
  if(g){renderGamePlanManager();renderLiveGame()}
}
...
function renderGamePlanManager(){
  ...
  select.innerHTML=available.length?'<option value="">Choose a play…</option>'+available.map(p=>`<option value="${p.id}">${esc(p.name)} (default #${p.number})</option>`).join(""):'<option value="">All active plays are assigned</option>';
...
async function persistVoiceCorrection(from,to){if(!SB||!cloudUser||!cloudLinked()||!isCloudStatkeeper())return;try{const {data:existing,error:readError}=await SB.from("team_voice_corrections").select("id,use_count").eq("team_id",S.cloud.teamId).eq("heard_text",from).maybeSingle();if(readError)throw readError;const payload={team_id:S.cloud.teamId,heard_text:from,resolved_value:{value:to},use_count:Number(existing?.use_count||0)+1,updated_at:new Date().toISOString()};const q=existing?.id?await SB.from("team_voice_corrections").update(payload).eq("id",existing.id):await SB.from("team_voice_corrections").insert(payload);if(q.error)throw q.error}catch(e){console.warn("Team voice correction cloud save failed",e)}}
