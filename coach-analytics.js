(function(){
  ...
  function renderOffense(ctx){
    const games=selectedGames(ctx.games,ctx.selection),metrics=metricSet(games),plays=metrics.off,rush=metrics.rush,pass=metrics.pass,total=plays.length,humanRead=staffContext(ctx.debriefs);
    ...
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>OFFENSE</h2><p>What moved the ball and when it worked.</p></div>
      ...
      <div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p><b>Data-supported:</b> ${esc(read||"Record more offensive plays to build a reliable situational read.")}</p>${humanRead?`<p class="staff-context"><b>Coach-provided context:</b> ${esc(humanRead)}</p>`:""}</div></div>`;
  }
  function renderDefense(ctx){
    const games=selectedGames(ctx.games,ctx.selection),plays=defense(allPlays(games)),humanRead=staffContext(ctx.debriefs),yards=plays.reduce((sum,p)=>sum+number(p.yards),0),positiveYards=plays.reduce((sum,p)=>sum+Math.max(0,number(p.yards)),0),takeaways=plays.filter(defensiveTakeaway).length,tfl=plays.filter(defensiveTfl).length,sacks=plays.filter(defensiveSack).length,stops=plays.filter(p=>number(p.yards)<=0).length,points=games.reduce((sum,g)=>sum+number(g.displayOppScore??g.oppScore),0);
    ...
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>DEFENSE</h2><p>Stops, disruption, and where opponents found space.</p></div>
      ...
      <div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p><b>Data-supported:</b> ${esc(read)}</p>${humanRead?`<p class="staff-context"><b>Coach-provided context:</b> ${esc(humanRead)}</p>`:""}</div></div>`;
  }
