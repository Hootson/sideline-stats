(function(){
  ...
  function generatedSection(ctx,key){
    return selectedGames(ctx.games,ctx.selection).length===1?String(ctx.generatedRead?.sections?.[key]||"").trim():"";
  }
  ...
  function renderOverview(ctx){
    ...
    const humanRead=staffContext(ctx.debriefs),generated=generatedSection(ctx,"overview");
    ...
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>GAME OVERVIEW</h2></div>
      <div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p>${esc(generated||coachRead(metrics))}</p>${!generated&&humanRead?`<p class="staff-context">Coach responses are still being collected. This read refreshes when the 24-hour window closes or everyone responds.</p>`:""}</div></div>
  ...
  function renderPlayCalls(ctx){
    ...
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO • ${playbookCount} PLAYBOOK CALLS • ${usedCount} USED</div><h2>PLAY CALLS</h2></div>
      ...
      <div class="coach-read compact"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p>${esc(generatedSection(ctx,"play_calls")||playCallRead(groups,selectedDown))}</p></div></div>
  ...
  const situationalDistanceBucket=play=>distance(play)<=3?"short":distance(play)<=7?"medium":"long";
  const firstDown=play=>play?.firstDown===true||play?.extras?.includes("First Down");
  const touchdown=play=>play?.extras?.includes("TD")||play?.extras?.includes("Touchdown");
  const playQuarter=play=>Math.min(4,Math.max(1,number(play?.quarter||play?.stateBefore?.quarter)||1));
  const defenseSuccessAllowed=play=>firstDown(play)||number(play?.yards)>=distance(play)*(down(play)===1?.4:down(play)===2?.6:1);
  const defensiveTakeaway=play=>Boolean(play?.interceptionPlayerId||play?.fumbleRecoveryPlayerId||play?.sub==="INT"||play?.sub==="Fumble Recovery");
  const defensiveTfl=play=>play?.sub==="TFL"||play?.tackleKind==="TFL"||number(play?.yards)<0;
  const defensiveSack=play=>play?.sub==="Sack";
  const rateClass=(rate,inverse=false)=>rate===null||rate===undefined?"low":(inverse?rate<=.2:rate>=.6)?"strong":(inverse?rate<=.35:rate>=.4)?"mixed":"weak";
  const analysisTable=(headers,rows,extraClass="")=>`<div class="analysis-table-wrap"><table class="analysis-table ${extraClass}"><thead><tr>${headers.map((h,i)=>`<th class="${i?"num":""}">${esc(h)}</th>`).join("")}</tr></thead><tbody>${rows.join("")}</tbody></table></div>`;
  function renderOffense(ctx){
    const games=selectedGames(ctx.games,ctx.selection),metrics=metricSet(games),plays=metrics.off,rush=metrics.rush,pass=metrics.pass,total=plays.length,humanRead=staffContext(ctx.debriefs),generated=generatedSection(ctx,"offense");
    const firstDowns=plays.filter(firstDown).length,touchdowns=plays.filter(touchdown).length;
    const buckets=[{key:"short",label:"SHORT",range:"1–3"},{key:"medium",label:"MEDIUM",range:"4–7"},{key:"long",label:"LONG",range:"8+"}].map(bucket=>{const list=plays.filter(p=>situationalDistanceBucket(p)===bucket.key),yards=list.reduce((sum,p)=>sum+playYards(p),0);return {...bucket,list,ypp:list.length?yards/list.length:0,rate:list.length?list.filter(successful).length/list.length:null}});
    const downs=[1,2,3,4].map(d=>{const list=plays.filter(p=>down(p)===d),runs=list.filter(p=>p.type==="Rush").length,yards=list.reduce((sum,p)=>sum+playYards(p),0),runRate=list.length?runs/list.length:null,rate=list.length?list.filter(successful).length/list.length:null;return {d,list,ypp:list.length?yards/list.length:0,runRate,rate,tendency:!list.length?"Low sample":runRate>=.7?"Run heavy":runRate<=.3?"Pass heavy":"Balanced"}});
    const best=downs.filter(x=>x.list.length).sort((a,b)=>b.ypp-a.ypp)[0],predictable=downs.find(x=>x.list.length>=3&&(x.runRate>=.8||x.runRate<=.2)),read=[`The offense averaged ${metrics.yardsPerPlay.toFixed(1)} yards per play.`,best&&`${ordinal(best.d)} down was most productive at ${best.ypp.toFixed(1)} yards per play.`,predictable&&`${ordinal(predictable.d)} down showed a ${predictable.runRate>=.8?"run":"pass"} tendency that opponents may anticipate.`].filter(Boolean).join(" ");
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>OFFENSE</h2><p>What moved the ball and when it worked.</p></div>
      <div class="coach-metric-grid"><div class="coach-metric"><strong>${Math.round(metrics.offYards)}</strong><span>TOTAL YARDS • ${total} PLAYS</span></div><div class="coach-metric"><strong>${metrics.yardsPerPlay.toFixed(1)}</strong><span>YARDS / PLAY</span></div><div class="coach-metric"><strong>${pct(metrics.successRate)}</strong><span>SUCCESS RATE</span></div><div class="coach-metric"><strong>${firstDowns}</strong><span>FIRST DOWNS • ${touchdowns} TD</span></div></div>
      <section class="coach-panel"><div class="coach-panel-heading"><h3>OFFENSIVE IDENTITY</h3><span>${metrics.explosives} explosive</span></div><div class="identity-split"><div><strong>${total?pct(rush.length/total):"0%"}</strong><span>Run • ${avg(rush.reduce((s,p)=>s+playYards(p),0),rush.length)} YPP</span></div><div><strong>${total?pct(pass.length/total):"0%"}</strong><span>Pass • ${avg(pass.reduce((s,p)=>s+playYards(p),0),pass.length)} YPA</span></div></div></section>
      <section class="coach-panel"><h3>SUCCESS BY DISTANCE</h3><div class="distance-metric-grid">${buckets.map(bucket=>`<div class="distance-metric ${rateClass(bucket.rate)}"><strong>${bucket.rate===null?"—":pct(bucket.rate)}</strong><span>${bucket.label} • ${bucket.range}</span><small>${bucket.list.length} play${bucket.list.length===1?"":"s"}<br>${bucket.ypp.toFixed(1)} yards/play</small></div>`).join("")}</div></section>
      <section class="coach-panel"><div class="coach-panel-heading"><h3>SITUATION BY DOWN</h3><span>SELF-SCOUT</span></div>${analysisTable(["Down","Run / Pass","YPP","Success","Tendency"],downs.map(row=>`<tr><td><b>${ordinal(row.d)}</b></td><td class="num">${row.list.length?`${pct(row.runRate)} / ${pct(1-row.runRate)}`:"—"}</td><td class="num">${row.list.length?row.ypp.toFixed(1):"—"}</td><td class="num">${row.rate===null?"—":pct(row.rate)}</td><td class="num"><span class="table-status ${row.tendency==="Balanced"?"strong":row.tendency==="Low sample"?"low":"weak"}">${row.tendency}</span></td></tr>`),"offense-down-table")}</section>
      <div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p>${esc(generated||read||"Record more offensive plays to build a reliable situational read.")}</p>${!generated&&humanRead?`<p class="staff-context">Coach responses are still being collected.</p>`:""}</div></div>`;
  }
  function renderDefense(ctx){
    const games=selectedGames(ctx.games,ctx.selection),plays=defense(allPlays(games)),humanRead=staffContext(ctx.debriefs),generated=generatedSection(ctx,"defense"),yards=plays.reduce((sum,p)=>sum+number(p.yards),0),positiveYards=plays.reduce((sum,p)=>sum+Math.max(0,number(p.yards)),0),takeaways=plays.filter(defensiveTakeaway).length,tfl=plays.filter(defensiveTfl).length,sacks=plays.filter(defensiveSack).length,stops=plays.filter(p=>number(p.yards)<=0).length,points=games.reduce((sum,g)=>sum+number(g.displayOppScore??g.oppScore),0);
    const depth=[{label:"Behind line",count:plays.filter(p=>number(p.yards)<0).length,cls:"behind"},{label:"No gain",count:plays.filter(p=>number(p.yards)===0).length,cls:"none"},{label:"1–3 yards",count:plays.filter(p=>number(p.yards)>=1&&number(p.yards)<=3).length,cls:"short"},{label:"4–9 yards",count:plays.filter(p=>number(p.yards)>=4&&number(p.yards)<=9).length,cls:"solid"},{label:"10+ yards",count:plays.filter(p=>number(p.yards)>=10).length,cls:"explosive"}];
    const downs=[1,2,3,4].map(d=>{const list=plays.filter(p=>down(p)===d),allowed=list.reduce((s,p)=>s+number(p.yards),0),rate=list.length?list.filter(defenseSuccessAllowed).length/list.length:null,havoc=list.filter(p=>defensiveTfl(p)||defensiveSack(p)).length;return {d,list,ypp:list.length?allowed/list.length:0,rate,havoc,takeaways:list.filter(defensiveTakeaway).length}});
    const quarters=[1,2,3,4].map(q=>{const list=plays.filter(p=>playQuarter(p)===q),allowed=list.reduce((s,p)=>s+number(p.yards),0),rate=list.length?list.filter(defenseSuccessAllowed).length/list.length:null;return {q,list,ypp:list.length?allowed/list.length:0,rate,takeaways:list.filter(defensiveTakeaway).length}});
    const oppRun=plays.filter(p=>p.sub==="Opponent Run"),oppPass=plays.filter(p=>p.sub!=="Opponent Run"),runYpp=oppRun.length?oppRun.reduce((s,p)=>s+number(p.yards),0)/oppRun.length:0,passYpp=oppPass.length?oppPass.reduce((s,p)=>s+number(p.yards),0)/oppPass.length:0,worst=quarters.filter(x=>x.list.length).sort((a,b)=>b.ypp-a.ypp)[0],read=`The defense allowed ${plays.length?(yards/plays.length).toFixed(1):"0.0"} yards per play and stopped ${plays.length?pct(stops/plays.length):"0%"} at or behind the line. ${worst?`Quarter ${worst.q} was the opponent’s most productive stretch at ${worst.ypp.toFixed(1)} yards per play.`:""}`;
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>DEFENSE</h2><p>Stops, disruption, and where opponents found space.</p></div>
      <div class="coach-metric-grid"><div class="coach-metric"><strong>${Math.round(points)}</strong><span>POINTS ALLOWED</span></div><div class="coach-metric"><strong>${plays.length?(positiveYards/plays.length).toFixed(1):"0.0"}</strong><span>YARDS / PLAY ALLOWED</span></div><div class="coach-metric"><strong>${plays.length?pct(stops/plays.length):"0%"}</strong><span>STOP RATE • ${stops} PLAYS</span></div><div class="coach-metric"><strong>${takeaways}</strong><span>TAKEAWAYS</span></div></div>
      <section class="coach-panel"><div class="coach-panel-heading"><h3>WHERE PLAYS ENDED</h3><span>${plays.length?pct((stops+depth[2].count)/plays.length):"0%"} held to 3 or less</span></div><div class="defense-depth-bar">${depth.map(x=>`<i class="${x.cls}" style="width:${plays.length?x.count/plays.length*100:0}%"></i>`).join("")}</div><div class="defense-depth-legend">${depth.map(x=>`<span><i class="${x.cls}"></i><b>${plays.length?pct(x.count/plays.length):"0%"}</b> ${x.label}</span>`).join("")}</div></section>
      <section class="coach-panel"><h3>SITUATIONAL DEFENSE</h3>${analysisTable(["Down","Plays","YPP","Success allowed","Read"],downs.map(row=>`<tr><td><b>${ordinal(row.d)}</b></td><td class="num">${row.list.length}</td><td class="num">${row.list.length?row.ypp.toFixed(1):"—"}</td><td class="num">${row.rate===null?"—":pct(row.rate)}</td><td class="num"><span class="table-status ${rateClass(row.rate,true)}">${row.rate===null?"Low sample":row.rate<=.1?"Elite":row.rate<=.25?"Strong":row.rate<=.4?"Watch":"Needs work"}</span></td></tr>`))}</section>
      <section class="coach-panel"><div class="coach-panel-heading"><h3>HAVOC BY DOWN</h3><span>${tfl} TFL • ${sacks} sacks</span></div><div class="havoc-chart">${downs.map(row=>{const rate=row.list.length?row.havoc/row.list.length:0;return `<div><b>${pct(rate)}</b><i style="height:${Math.max(3,rate*150)}px"></i><span>${ordinal(row.d)}</span><small>${row.havoc} plays</small></div>`}).join("")}</div></section>
      <section class="coach-panel"><div class="coach-panel-heading"><h3>OPPONENT OFFENSE BY QUARTER</h3><span>Lower is better</span></div><div class="quarter-metric-grid">${quarters.map(row=>`<div class="quarter-metric ${rateClass(row.rate,true)}"><b>Q${row.q}</b><strong>${row.list.length?row.ypp.toFixed(1):"—"}</strong><span>YPP allowed</span><small>${row.list.length} plays<br>${row.rate===null?"—":pct(row.rate)} success<br>${row.takeaways} takeaway${row.takeaways===1?"":"s"}</small></div>`).join("")}</div></section>
      <section class="coach-panel"><h3>RUN VS PASS</h3><div class="identity-split"><div><strong>${runYpp.toFixed(1)}</strong><span>Run YPP allowed • ${oppRun.length} plays</span></div><div><strong>${passYpp.toFixed(1)}</strong><span>Pass YPP allowed • ${oppPass.length} plays</span></div></div></section>
      <div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p>${esc(generated||read)}</p>${!generated&&humanRead?`<p class="staff-context">Coach responses are still being collected.</p>`:""}</div></div>`;
  }
  ...
  function renderPlayers(ctx){
    ...
    const generated=generatedSection(ctx,"personnel");
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>PLAYERS</h2></div>${tabs}${generated?`<div class="coach-read compact"><div class="coach-read-icon">↗</div><div><strong>COACH READ</strong><p>${esc(generated)}</p></div></div>`:""}<section class="coach-panel coach-player-list">${rows.length?rows.join(""):'<div class="coach-empty">No player data is available in this view.</div>'}</section>`;
  }
  ...
  function renderDebrief(ctx){
    ...
    const cycle=ctx.debriefCycle,assignment=ctx.debriefAssignment,closed=!cycle||cycle.status!=="open"||Date.parse(cycle.deadline_at)<=Date.now(),responded=assignment&&assignment.status!=="pending",disabled=closed||responded;
    const deadline=cycle?new Date(cycle.deadline_at).toLocaleString():"Not open yet",responseLabel=assignment?.status==="submitted"?"Submitted":assignment?.status==="skipped"?"Skipped":closed?"Window closed":"Response pending";
    return `<div class="coach-title-block"><div class="coach-kicker">COACH PRO</div><h2>GAME DEBRIEF</h2></div>
      <section class="coach-panel debrief-window"><div class="coach-panel-heading"><h3>${esc(responseLabel)}</h3><span>${cycle?`${number(cycle.responded_coach_count)} of ${number(cycle.expected_coach_count)} complete`:"Waiting for finalization"}</span></div><p class="muted">${cycle?`Deadline: ${esc(deadline)}. The Coaching Read refreshes after every coach submits or skips, or when the 24-hour window closes.`:"This debrief opens once the statkeeper finalizes the game."}</p>${ctx.generatedRead?`<p><b>Coaching Read refreshed ${esc(new Date(ctx.generatedRead.generated_at).toLocaleString())}.</b></p>`:""}${ctx.pushAvailable?'<button class="btn ghost" id="enableCoachNotifications" type="button">Enable App Notifications</button>':""}</section>
      <section class="coach-panel"><h3>YOUR DEBRIEF • WEEK ${esc(game.week)} VS ${esc(game.opponent)}</h3>
        <fieldset ${disabled?"disabled":""}><div class="debrief-ratings"><label>Energy<select id="debriefEnergy"><option value="">—</option>${[1,2,3,4,5].map(n=>`<option value="${n}" ${number(own.energy_rating)===n?"selected":""}>${n}</option>`).join("")}</select></label><label>Execution<select id="debriefExecution"><option value="">—</option>${[1,2,3,4,5].map(n=>`<option value="${n}" ${number(own.execution_rating)===n?"selected":""}>${n}</option>`).join("")}</select></label></div>
        ...
        <div class="debrief-actions"><button class="btn ghost" id="saveDebriefDraft">Save Draft</button><button class="btn ghost" id="skipDebrief">Skip Debrief</button><button class="btn" id="submitDebrief">Submit Debrief</button></div></fieldset><div class="muted debrief-status">${own.updated_at?`Last saved ${esc(new Date(own.updated_at).toLocaleString())}`:"Not saved yet"}</div>
      </section>${ctx.generatedRead?.sections?.practice_priorities?`<div class="coach-read"><div class="coach-read-icon">↗</div><div><strong>PRACTICE PRIORITIES</strong><p>${esc(ctx.generatedRead.sections.practice_priorities)}</p></div></div>`:""}`;
  }
  function render(tab,ctx){
    if(tab==="playcalls")return renderPlayCalls(ctx);
    if(tab==="offense")return renderOffense(ctx);
    if(tab==="defense")return renderDefense(ctx);
