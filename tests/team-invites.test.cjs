...
const access=fs.readFileSync('commercial-access.js','utf8');
...
assert.match(app,/redirectUrl\.searchParams\.set\(new URLSearchParams\(location\.search\)\.has\("coachInvite"\)\?"coachInvite":"teamInvite",inviteToken\)/,'email confirmation must preserve the correct invitation route');
...
assert.match(app,/function releaseCoachInviteUrl\(token\)[\s\S]*searchParams\.set\("coachInvite",token\)/,'coach links must use a distinct route that cannot be mistaken for a parent link');
assert.match(app,/function releaseViewerInviteUrl\(token\)[\s\S]*parent-viewer\.html/,'parent links must open the public parent viewer directly');
assert.match(access,/get_public_team_viewer/,'legacy shared invitation URLs must verify the token type before redirecting');
assert.doesNotMatch(access,/if\(token && [\s\S]*location\.replace\(`\.\/parent-viewer\.html/,'coach tokens must never be unconditionally redirected to Parent Viewer');
